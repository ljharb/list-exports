				...
				st.test(`${fixture} (${category}): native node resolution`, { skip: skipReason }, async (s2t) => {
					...
					if (hasDynamicImport()) {
						await entries(exportResults.import).reduce(async (prev, [specifier, file]) => {
							...
							const supportsImportAssert = true; // TODO: make an entry point on hasDynamicImport
							const supportsImportWith = false; // TODO: make an entry point on hasDynamicImport when node supports this
							try {
								if (ext === '.json') {
