...
function validateExportsRHSObject(rhs, problems) {
	const exportKeys = keys(rhs);
	const starts = new Set(map(exportKeys, (x) => $charAt(String(x), 0)));
	if (starts.has('.') && starts.size !== 1) {
		problems.add('package `exports` is invalid; either all keys in an object, or no keys in it, must start with `.`');
	}
	if (some(exportKeys, (x) => includes(x, 'node_modules'))) {
		problems.add('package `exports` is invalid; keys may not contain `node_modules`');
	}
	return starts.has('.') ? 'files' : 'conditions';
}
...
async function forEachSubfile(realFile, {
	...
	if (canImport) {
		if (!options.skipMainDot && mains && mains.get('.') === realFile && !has(packageExports || {}, '.')) {
			safeSet(tree.import, '.', realFile);
		}
		if (!has(packageExports || {}, fakeFile)) {
	...
	if (canRequire) {
		...
		if (mains && mains.get('.') === realFile) {
			if (!options.skipMainDot && !has(packageExports || {}, '.')) {
		...
		if (ext !== '.cjs' && ext !== '.mjs') {
			...
			if (!has(packageExports || {}, extlessFile)) {
...
function addFullPath(
	...
	if (startsWith(rhs, './') && !includes(rhs, 'node_modules')) {
		...
		problems.add(`\`exports[${lhs}]\`: ${rhs} must start with \`./\` and must not contain \`node_modules\``);
	}
	return false;
}
...
async function forEachExportEntry([lhs, maybeRHS], conditionChain, {
	...
	return asyncReduce($concat([], maybeRHS), async (prev, rhs) => {
		...
		const rhsType = validateExportsRHSObject(rhs, problems);
		if (rhsType === 'files') {
	...
	if (typeof pkgData.exports === 'string') {
		...
		await asyncReduce(exportValues, async (prev, value) => { // TODO: fixtures for nested arrays in "broken"
			...
			if (value && typeof value === 'object') {
				const topLevelObjectType = validateExportsRHSObject(value, problems);
				if (topLevelObjectType === 'conditions') {
					if (category === 'broken') {
				...
				if (topLevelObjectType !== 'files') {
					throw new TypeError(`unknown top-level exports object type found: ${topLevelObjectType}`);
				}
				return asyncForEach(
					entries(value),
					([lhs, rhs]) => {
